...
def test_repair_app_rejects_empty_app_id():
    with pytest.raises(ValueError, match="non-empty app id"):
        make_adapter().repair_app("")


def test_probe_does_not_follow_redirects(monkeypatch: pytest.MonkeyPatch):
    import urllib.error
