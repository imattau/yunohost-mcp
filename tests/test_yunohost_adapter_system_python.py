...
def test_updates_refresh_calls_system_python(monkeypatch: pytest.MonkeyPatch):
    captured = {}
    ...
    result = YunohostAdapter(settings=_settings()).updates_refresh(target="apps")

    assert captured == {
        "module_name": "yunohost.tools",
        "attr": "tools_update",
        "kwargs": {"target": "apps"},
    }
