...
- `app-admin`: operator plus normal app install/upgrade/remove, app config-panel writes, `app_change_url`, domain writes (including certificate install), user writes/deletion, backup restore/delete, `system_upgrade`, `migrations_run`, `firewall_open`/`firewall_close`/`firewall_reload`, `settings_set`, and `regenconf_apply`. High-risk actions still require policy confirmation and, where configured, owner approval — for the last four especially, that owner co-signature (a *different* identity) is the real per-call safety gate, not the role.
