...
These tools ride through a `yunohost-mcp-connect` bridge (an entry in this harness's own MCP client config), not a direct connection to the server — its health is worth checking in its own right, separately from the YunoHost server it forwards to:
...
Confirmed live against a real `*.nohost.me` domain: for a domain whose registrar is YunoHost itself (any `*.nohost.me`/`*.noho.st`/`*.ynh.fr` domain not delegated to a third-party registrar), `domain_dns_push_preview` is not actually a dry run - upstream YunoHost's `domain_dns_push` short-circuits to a live DynDNS IP re-registration before its own dry-run check ever runs. This is idempotent and harmless (the same thing YunoHost's automatic DynDNS refresh does periodically), but don't describe a `changes: {}` response from one of these domains as "nothing to change" - it means the short-circuit ran, not that a diff was computed. There's no real preview available for this registrar type at all.
